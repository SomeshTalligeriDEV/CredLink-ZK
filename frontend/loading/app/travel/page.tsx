...
export default function TravelPage() {
  ...
  useEffect(() => {
    ...
    const generateQR = async () => {
      try {
        ...
        const url = await QRCode.toDataURL(passportData, {
          width: 280,
          margin: 2,
          color: { dark: "#F5A623", light: "#0B0D10" },
        });
        setQrDataUrl(url);
      } catch (e) {
    ...
    generateQR();
  }, [address, creditData, mocaVerified]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-[#F5A623]/10 border border-[#F5A623]/30 rounded-full px-4 py-2 mb-4">
            <span className="text-[#F5A623] text-sm font-bold">TRAVEL MODE</span>
          </div>
          <h1 className="text-4xl font-bold mb-2">
            Your <span className="text-[#F5A623]">Credit Passport</span>
          </h1>
          <p className="text-[#B0B3B8]">
            Your credit reputation travels with you — anywhere in the world
          </p>
          <p className="text-[#6B6F76] text-sm mt-2">
            Moving abroad? Show this to lenders. Your on-chain history speaks for you.
          </p>
        </div>

        {/* Real Story Banner */}
        <div className="bg-[#14171C] border border-[#F5A623]/20 rounded-2xl shadow-card p-4 mb-8 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[#F5A623] text-xs font-bold mt-2">
        ...
        {activeTab === "passport" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Passport Card */}
            <div
              className="bg-[#14171C] border-2 rounded-2xl shadow-card p-6 relative overflow-hidden"
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <div className="text-xs text-[#6B6F76] mb-1">CREDLINK ZK</div>
                  <div className="text-lg font-bold text-[#F5A623]">
                    Credit Passport
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-[#6B6F76]">CHAIN</div>
              ...
              <div className="flex justify-center mb-6">
                <div className="relative w-32 h-32">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <div className="text-xs text-[#6B6F76]">/1000</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <span className="text-[#B0B3B8] text-sm">Tier</span>
                  <span
                ...
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <span className="text-[#B0B3B8] text-sm">Collateral Required</span>
                  <span className="font-bold text-[#F5A623]">
                    {creditData.collateralRatio}%
                  </span>
                </div>
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <span className="text-[#B0B3B8] text-sm">ZK Verified</span>
                  <span
                    className={`text-sm font-bold ${
                      creditData.zkVerified ? "text-green-400" : "text-[#6B6F76]"
                    }`}
                  >
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[#B0B3B8] text-sm">Moca Identity</span>
                  <span
                    className={`text-sm font-bold ${
                      mocaVerified ? "text-green-400" : "text-yellow-400"
                    }`}
                  >
                    {mocaVerified ? "BOUND" : "NOT BOUND"}
                  </span>
                </div>
              </div>

              {/* Wallet Address */}
              <div className="mt-4 bg-[#0B0D10] rounded-xl p-3">
                <div className="text-xs text-[#6B6F76] mb-1">WALLET</div>
                <div className="text-xs font-mono text-[#B0B3B8] break-all">
            ...
            {/* QR Code */}
            <div className="bg-[#14171C] border border-white/5 rounded-2xl shadow-card p-6 flex flex-col items-center justify-center">
              <h3 className="font-bold text-lg mb-2">Scan to Verify</h3>
              <p className="text-[#B0B3B8] text-sm text-center mb-6">
                Show this QR to any lender worldwide. They can verify your credit
                score instantly on-chain.
              </p>

              {qrDataUrl ? (
                <div className="bg-[#0B0D10] p-3 rounded-xl border border-[#F5A623]/20">
                ...
                <div className="w-48 h-48 bg-white/10 rounded-xl flex items-center justify-center">
                  <span className="text-[#6B6F76] text-sm">Connect wallet to generate QR</span>
                </div>
              )}
              ...
              <button
                ...
                className="mt-4 w-full py-2 border border-[#F5A623]/40 text-[#F5A623] rounded-full text-sm font-bold hover:bg-[#F5A623]/10 transition-all"
              >
                Download QR Code
              </button>
            </div>
          ...
          <div className="bg-[#14171C] border border-white/5 rounded-2xl shadow-card p-6">
            ...
            <div className="flex gap-3 mb-6">
              <input
                ...
                className="flex-1 bg-[#0B0D10] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono text-white placeholder-gray-600 focus:outline-none focus:border-[#F5A623]"
              />
              <button
                onClick={handleVerify}
                className="bg-[#F5A623] text-black font-bold px-6 py-3 rounded-full hover:shadow-gold-glow transition-all"
              >
                Verify
              </button>
            </div>
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-[#B0B3B8]">Wallet</span>
                    <span className="font-mono text-xs">
                      {verifyResult.walletAddress}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#B0B3B8]">Moca Verified</span>
                    <span
                      className={
                        verifyResult.mocaVerified
                  ...
                  <div className="flex justify-between">
                    <span className="text-[#B0B3B8]">Can Borrow</span>
                    <span
                      className={
        ...
        {activeTab === "connect" && (
          <div className="bg-[#14171C] border border-white/5 rounded-2xl shadow-card p-6">
            ...
            <div className="bg-[#F5A623]/5 border border-[#F5A623]/20 rounded-xl p-4 mb-6">
              <div className="text-sm font-bold text-[#F5A623] mb-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-[#0B0D10] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono text-[#B0B3B8]">
              ...
              <button
                ...
                disabled={mocaLoading || !address || mocaVerified}
                className="w-full bg-[#F5A623] text-black font-bold py-3 rounded-full hover:shadow-gold-glow transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {mocaLoading ? "Verifying..." : mocaVerified ? "Identity Already Bound" : "Login with Moca to Bind Identity"}
              </button>
            </div>
            ...
            {mocaVerified && (
              <div className="mt-4 bg-green-400/10 border border-green-400/20 rounded-xl p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  {identityHash && (
                    <div className="text-xs font-mono text-[#6B6F76] mt-1">
                      Hash: {identityHash.slice(0, 10)}...{identityHash.slice(-8)}
                    </div>
                  )}
                </div>
              </div>
        ...
        {activeTab === "migrate" && (
          <div className="bg-[#14171C] border border-white/5 rounded-2xl shadow-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                <label className="text-sm text-[#B0B3B8] mb-2 block">Origin Country</label>
                <select
                  value={originCountry}
                  onChange={(e) => setOriginCountry(e.target.value)}
                  className="w-full bg-[#0B0D10] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#F5A623]"
                >
              ...
              <div>
                <label className="text-sm text-[#B0B3B8] mb-2 block">Destination Country</label>
                <select
                  value={destCountry}
            ...
            <button
              ...
              className="w-full bg-[#F5A623] text-black font-bold py-3 rounded-full hover:shadow-gold-glow transition-all disabled:opacity-50 disabled:cursor-not-allowed mb-6"
            >
              Simulate Migration
            </button>
            ...
            {simResult && (
              <div className="space-y-4">
                <div className="bg-[#F5A623]/5 border border-[#F5A623]/20 rounded-xl p-4">
                  <div className="text-sm font-bold text-[#F5A623] mb-3">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center border-b border-white/5 pb-2">
                      <span className="text-[#B0B3B8] text-sm">Rental Deposit Reduction</span>
                      <span className={`font-bold text-sm ${simResult.rentalDepositReduction > 0 ? "text-green-400" : "text-[#6B6F76]"}`}>
                        {simResult.rentalDepositReduction > 0 ? `${simResult.rentalDepositReduction}% OFF` : "Not eligible"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center border-b border-white/5 pb-2">
                      <span className="text-[#B0B3B8] text-sm">Microloan Pre-Approval</span>
                    ...
                    <div className="flex justify-between items-center border-b border-white/5 pb-2">
                      <span className="text-[#B0B3B8] text-sm">Credit Transfer Speed</span>
                      <span className="font-bold text-sm text-[#00D2D3]">{simResult.creditTransferTime}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-white/5 pb-2">
                      <span className="text-[#B0B3B8] text-sm">Borrowing Power</span>
                      <span className="font-bold text-sm text-[#F5A623]">{simResult.borrowingPower}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#B0B3B8] text-sm">Interest Rate</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-red-400/5 border border-red-400/20 rounded-xl p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="text-[#B0B3B8]">&#10007; No credit history abroad</div>
                      <div className="text-[#B0B3B8]">&#10007; 3-12 month waiting period</div>
                      <div className="text-[#B0B3B8]">&#10007; Full deposit on rental</div>
                      <div className="text-[#B0B3B8]">&#10007; No borrowing access</div>
                    </div>
                  </div>

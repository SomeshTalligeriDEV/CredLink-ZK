// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "@openzeppelin/contracts/access/AccessControl.sol";

/**
 * @title ICreditScoreZKForVerifier
 * @notice Minimal interface for the CreditScoreZK contract used by ZKVerifier.
 */
interface ICreditScoreZKForVerifier {
    function updateScoreFromZK(address user, uint256 newScore) external;
}

/**
 * @title ZKVerifier
 * @author CredLink ZK Team
 * @notice Groth16 ZK proof verifier for the CredLink behavioral credit scoring
 *         system. Accepts a proof and three public signals (walletAgeValid,
 *         repaymentValid, scoreOutput) and, upon successful verification,
 *         updates the user's on-chain credit score via CreditScoreZK.
 *
 * INVARIANTS:
 *   INV-1: A score update on CreditScoreZK only occurs when proof verification
 *          returns true (mock or production).
 *   INV-2: publicSignals[2] (scoreOutput) is always validated to be <= 1000.
 *   INV-3: zkProofCount[user] is monotonically increasing.
 *   INV-4: Badge events are emitted exactly once per threshold crossing.
 *
 * ARCHITECTURE:
 *   ZKVerifier is the only contract authorized to set absolute scores on
 *   CreditScoreZK (via VERIFIER_ROLE). It acts as the trust boundary between
 *   off-chain ZK computation and on-chain state.
 *
 * ATTACK RESISTANCE:
 *   - Only admin can toggle production mode.
 *   - Production mode placeholder returns false to prevent accidental bypass.
 *   - Score validated within range before forwarding.
 *   - Anyone can call verifyAndUpdateScore (permissionless proof submission).
 *
 * UPGRADE PATH:
 *   - Replace _groth16Verify body with snarkjs-generated verifier.
 *   - Add proof nullifier tracking to prevent replay attacks.
 *   - Add timestamp-based proof freshness requirement.
 */
contract ZKVerifier is AccessControl {
    // -----------------------------------------------------------------------
    //  Custom Errors
    // -----------------------------------------------------------------------

    /// @dev Reverts when a zero address is provided.
    error ZeroAddress();

    /// @dev Reverts when proof verification fails.
    error InvalidProof();

    // -----------------------------------------------------------------------
    //  State
    // -----------------------------------------------------------------------

    /// @notice Reference to the CreditScoreZK contract.
    ICreditScoreZKForVerifier public immutable creditScoreZK;

    /**
     * @notice When false (default), proof verification uses a mock that only
     *         checks public signals. Set to true for real Groth16 verification.
     */
    bool public productionMode;

    // -----------------------------------------------------------------------
    //  ZK Proof Gamification
    // -----------------------------------------------------------------------

    /// @notice Number of successfully verified ZK proofs per user.
    mapping(address => uint256) public zkProofCount;

    /// @notice Badge tier thresholds.
    uint256 public constant BADGE_BRONZE = 1;
    uint256 public constant BADGE_SILVER = 3;
    uint256 public constant BADGE_GOLD = 5;
    uint256 public constant BADGE_PLATINUM = 10;

    // -----------------------------------------------------------------------
    //  Events
    // -----------------------------------------------------------------------

    /// @notice Emitted when a ZK proof is successfully verified.
    /// @param user  The user whose score was updated.
    /// @param score The new credit score derived from the proof.
    event ProofVerified(address indexed user, uint256 score);

    /// @notice Emitted when production mode is toggled.
    /// @param enabled True if production mode is now enabled.
    event ProductionModeUpdated(bool enabled);

    /// @notice Emitted when a user earns a new ZK badge level.
    /// @param user       The user earning the badge.
    /// @param proofCount The user's total proof count at badge time.
    /// @param badgeLevel The badge name.
    event ZKBadgeEarned(address indexed user, uint256 proofCount, string badgeLevel);

    // -----------------------------------------------------------------------
    //  Constructor
    // -----------------------------------------------------------------------

    /**
     * @notice Deploys the ZKVerifier with a reference to the CreditScoreZK contract.
     * @param _creditScoreZK Address of the deployed CreditScoreZK contract.
     */
    constructor(address _creditScoreZK) {
        if (_creditScoreZK == address(0)) revert ZeroAddress();

        creditScoreZK = ICreditScoreZKForVerifier(_creditScoreZK);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    // -----------------------------------------------------------------------
    //  External — Proof Verification
    // -----------------------------------------------------------------------

    /**
     * @notice Verifies a Groth16 ZK proof and, if valid, updates the user's
     *         credit score on-chain.
     *
     * @dev Public signals layout:
     *        publicSignals[0] = walletAgeValid   (1 = valid)
     *        publicSignals[1] = repaymentValid   (1 = valid)
     *        publicSignals[2] = scoreOutput      (0-1000)
     *
     *      In mock mode the proof components (a, b, c) are ignored.
     *
     * @param a             Groth16 proof element A (2 field elements).
     * @param b             Groth16 proof element B (2x2 field elements).
     * @param c             Groth16 proof element C (2 field elements).
     * @param publicSignals The three public inputs.
     * @param user          The address whose credit score should be updated.
     */
    function verifyAndUpdateScore(
        uint256[2] calldata a,
        uint256[2][2] calldata b,
        uint256[2] calldata c,
        uint256[3] calldata publicSignals,
        address user
    ) external {
        if (user == address(0)) revert ZeroAddress();

        bool isValid = productionMode
            ? _groth16Verify(a, b, c, publicSignals)
            : _mockVerify(publicSignals);

        if (!isValid) revert InvalidProof();

        // Forward verified score to CreditScoreZK.
        creditScoreZK.updateScoreFromZK(user, publicSignals[2]);

        // Gamification: increment proof count and check badge thresholds.
        unchecked { zkProofCount[user] += 1; }
        uint256 count = zkProofCount[user];

        if (count == BADGE_PLATINUM) {
            emit ZKBadgeEarned(user, count, "Platinum");
        } else if (count == BADGE_GOLD) {
            emit ZKBadgeEarned(user, count, "Gold");
        } else if (count == BADGE_SILVER) {
            emit ZKBadgeEarned(user, count, "Silver");
        } else if (count == BADGE_BRONZE) {
            emit ZKBadgeEarned(user, count, "Bronze");
        }

        emit ProofVerified(user, publicSignals[2]);
    }

    // -----------------------------------------------------------------------
    //  Admin
    // -----------------------------------------------------------------------

    /**
     * @notice Toggles production mode for proof verification.
     * @dev Only callable by DEFAULT_ADMIN_ROLE.
     * @param enabled True to enable production Groth16 verification.
     */
    function setProductionVerifier(bool enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
        productionMode = enabled;
        emit ProductionModeUpdated(enabled);
    }

    // -----------------------------------------------------------------------
    //  External — ZK Badge Views
    // -----------------------------------------------------------------------

    /**
     * @notice Returns the ZK badge level for a user based on their proof count.
     * @param user The address to query.
     * @return badgeLevel The badge name ("None", "Bronze", "Silver", "Gold", "Platinum").
     * @return proofCount The total number of verified proofs.
     */
    function getZKBadgeLevel(
        address user
    ) external view returns (string memory badgeLevel, uint256 proofCount) {
        proofCount = zkProofCount[user];
        if (proofCount >= BADGE_PLATINUM) {
            badgeLevel = "Platinum";
        } else if (proofCount >= BADGE_GOLD) {
            badgeLevel = "Gold";
        } else if (proofCount >= BADGE_SILVER) {
            badgeLevel = "Silver";
        } else if (proofCount >= BADGE_BRONZE) {
            badgeLevel = "Bronze";
        } else {
            badgeLevel = "None";
        }
    }

    // -----------------------------------------------------------------------
    //  Internal — Verification Logic
    // -----------------------------------------------------------------------

    /**
     * @dev Mock verification for development. Returns true if walletAgeValid == 1,
     *      repaymentValid == 1, and scoreOutput <= 1000.
     * @param publicSignals The three public inputs.
     * @return True if the mock checks pass.
     */
    function _mockVerify(
        uint256[3] calldata publicSignals
    ) internal pure returns (bool) {
        return publicSignals[0] == 1 && publicSignals[1] == 1 && publicSignals[2] <= 1000;
    }

    /**
     * @dev Placeholder for the real Groth16 verifier. Returns false to prevent
     *      accidental use without a real verifier. Replace with snarkjs output.
     * @param a             Groth16 proof element A.
     * @param b             Groth16 proof element B.
     * @param c             Groth16 proof element C.
     * @param publicSignals The three public inputs.
     * @return True if the proof is valid.
     */
    function _groth16Verify(
        uint256[2] calldata a,
        uint256[2][2] calldata b,
        uint256[2] calldata c,
        uint256[3] calldata publicSignals
    ) internal pure returns (bool) {
        // Suppress unused variable warnings. Production: replace entire body.
        a; b; c; publicSignals;
        return false;
    }
}
